...
// -------------- ADD THIS --------------
const API_KEY = "AIzaSyD2wAuGZC6W8dNSZkOJo6lUOHtTQBd7C1A";  
//---------------------------------------
...
export default function SoilHealthTracker() {
  ...
  const [fileName, setFileName] = useState("");
  const [result, setResult] = useState(null);
  console.log(result,"map 1");
  console.log(result?.extractedData,"map 1");
  console.log(result?.extractedData?.pH,"map 1");
  ...
  const validateForm = () => {
    ...
    if (!formData.pH || formData.pH < 0 || formData.pH > 14) newErrors.pH = "pH must be between 0 and 14";
    if (!formData.EC || formData.EC < 0) newErrors.EC = "EC must be a positive number";
    if (!formData.N || formData.N < 0) newErrors.N = "Nitrogen must be a positive number";
    if (!formData.P || formData.P < 0) newErrors.P = "Phosphorus must be a positive number";
    if (!formData.K || formData.K < 0) newErrors.K = "Potassium must be a positive number";
  ...
  // ---------------- GEMINI API FILE PARSER -------------------
  const extractDataFromGemini = async (text) => {
    const prompt = `
Extract the following fields from this soil lab report text:
- pH
- EC (Electrical Conductivity)
- N (Nitrogen)
- P (Phosphorus)
...
${text}
    ...
    try {
      const textWithBlock = data.candidates[0].content.parts[0].text;

      // Strip json and  from the response
      const cleanText = textWithBlock.replace(/json/g, "").replace(g, "").trim();
  ...
  const onSubmitFile = async () => {
    ...
    try {
      const text = await file.text(); // Convert file to text for Gemini
      const extracted = await extractDataFromGemini(text);

      if (!extracted) {
        alert("Gemini could not extract data.");
        return;
      }
  ...
  const onSubmitManual = async (e) => {
    ...
    try {
      ...
      const avgPH = parseFloat(formData.pH) || 0;
      const soilHealth =
        avgPH >= 6 && avgPH <= 7.5
          ? "Good"
          : avgPH > 7.5
      ...
      console.error(err);
      alert("Error processing manual data.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-gradient-to-r from-green-500 to-blue-500 p-6">
            <div className="flex gap-4 justify-center">
              <button
                ...
                className={`px-6 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center gap-2 ${
                  mode === "manual" ? "bg-white text-green-600 shadow-lg scale-105" : "bg-white/20 text-white hover:bg-white/30"
                }`}
              >
                <TrendingUp className="w-5 h-5" />
                Track Manually
              ...
              <button
                ...
                className={`px-6 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center gap-2 ${
                  mode === "upload" ? "bg-white text-blue-600 shadow-lg scale-105" : "bg-white/20 text-white hover:bg-white/30"
                }`}
              >
                <Upload className="w-5 h-5" />
                Upload Lab Report
          ...
          <div className="p-8">
              ...
              <div className="space-y-5">
                <div className="grid md:grid-cols-2 gap-5">
                  ...
                  {["pH","EC","N","P","K"].map((field) => (
                    <div key={field} className={field==="K"?"md:col-span-2":""}>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">{field==="pH"?"pH Level":field==="EC"?"EC (Electrical Conductivity)":field==="N"?"Nitrogen (N)":field==="P"?"Phosphorus (P)":"Potassium (K)"}</label>
                      <input
                        type="number"
                        name={field}
                        step="0.01"
                        ...
                        onChange={handleInputChange}
                        placeholder={field==="pH"?"0.00 - 14.00":"mg/kg"}
                        className={`w-full border-2 px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition-all ${
                ...
                >
                  ...
                  <><CheckCircle2 className="w-5 h-5"/>Analyze Soil Health</>}
                </button>
              </div>
            )}
            ...
            {mode === "upload" && (
              <div className="space-y-5">
                <div className="border-3 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-blue-500 transition-all duration-300 bg-gray-50">
                  <Upload className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                  <p className="text-gray-600 mb-4">Upload your lab report in CSV or Excel format</p>
                  <input
                    type="file"
                    accept=".csv, .xlsx, .xls"
                    onChange={handleFileChange}
                  ...
                  <p className="text-xs text-gray-500 mt-4">Supported formats: CSV, XLSX, XLS (Max 5MB)</p>
                </div>
              ...
              <div className="mt-8 p-6 border-2 border-green-200 rounded-xl bg-gradient-to-br from-green-50 to-blue-50 animate-fadeIn">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div className="bg-white p-4 rounded-lg shadow text-center">
                    <p className="text-sm text-gray-600 mb-1">Average pH</p>
                    <p className="text-2xl font-bold text-green-600">{result.analysis.averagePH}</p>
                    {/* <p className="text-2xl font-bold text-green-600">{result.extracted.pH}</p> */}
                  </div>
                  <div className="bg-white p-4 rounded-lg shadow text-center">
                    <p className="text-sm text-gray-600 mb-1">Soil Health</p>
                    <p className="text-2xl font-bold text-green-600">{result.analysis.soilHealth}</p>
        ...
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-6">
          ...
          <div className="grid md:grid-cols-3 gap-4 text-sm text-blue-800">
            <div>
              <p className="font-semibold">Optimal pH:</p>
              <p>{result?.extractedData?.pH}</p>
            </div>
            <div>
              <p className="font-semibold">Nitrogen (N):</p>
              <p>{result?.extractedData?.N}</p>
            </div>
            <div>
              <p className="font-semibold">Phosphorus (P):</p>
              <p>{result?.extractedData?.P}</p>
            </div>
             <div>
              <p className="font-semibold">Potassium (K):</p>
              <p>{result?.extractedData?.K}</p>
            </div>
          </div>
        </div>
      </div>
